...
public class ICDCCLWorksurface extends ICDBasicWorksurface
{
    public ICDCCLWorksurface(final String s, final TypeObject typeObject, final OptionObject optionObject) {
        super(s, typeObject, optionObject);
    }
    
    public TransformableEntity buildClone(final ICDCCLWorksurface icdcclWorksurface) {
        super.buildClone(icdcclWorksurface);
        return (TransformableEntity)icdcclWorksurface;
    }
    
    public TransformableEntity buildClone2(final ICDCCLWorksurface icdcclWorksurface) {
        super.buildClone2(icdcclWorksurface);
        return (TransformableEntity)icdcclWorksurface;
    }
    
    public EntityObject frameClone(final EntityObject entityObject) {
        return this.buildFrameClone(new ICDCCLWorksurface(this.getId(), this.currentType, this.currentOption), entityObject);
    }
    
    @Override
    public Object clone() {
        return this.buildClone(new ICDCCLWorksurface(this.getId(), this.currentType, this.currentOption));
    }
    
    public EntityObject buildFrameClone(final ICDCCLWorksurface icdcclWorksurface, final EntityObject entityObject) {
        super.buildFrameClone((TransformableTriggerUser)icdcclWorksurface, entityObject);
        return (EntityObject)icdcclWorksurface;
    }
