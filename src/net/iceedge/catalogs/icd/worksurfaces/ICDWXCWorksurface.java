...
public class ICDWXCWorksurface extends ICDCornerWorksurface
{
    private float slantOffsetX;
    private float slantOffsetY;
    
    public ICDWXCWorksurface(final String s, final TypeObject typeObject, final OptionObject optionObject) {
        super(s, typeObject, optionObject);
        this.slantOffsetX = 9.0f;
        this.slantOffsetY = 25.0f;
    }
    
    public TransformableEntity buildClone(final ICDWXCWorksurface icdwxcWorksurface) {
        super.buildClone(icdwxcWorksurface);
        return (TransformableEntity)icdwxcWorksurface;
    }
    
    public TransformableEntity buildClone2(final ICDWXCWorksurface icdwxcWorksurface) {
        super.buildClone2(icdwxcWorksurface);
        return (TransformableEntity)icdwxcWorksurface;
    }
    
    public EntityObject frameClone(final EntityObject entityObject) {
        return this.buildFrameClone(new ICDWXCWorksurface(this.getId(), this.currentType, this.currentOption), entityObject);
    }
    
    @Override
    public Object clone() {
        return this.buildClone(new ICDWXCWorksurface(this.getId(), this.currentType, this.currentOption));
    }
    
    public EntityObject buildFrameClone(final ICDWXCWorksurface icdwxcWorksurface, final EntityObject entityObject) {
        super.buildFrameClone(icdwxcWorksurface, entityObject);
        return (EntityObject)icdwxcWorksurface;
    }
    ...
    public float getLeftDepth() {
        return this.getAttributeValueAsFloat("ICD_Scalable_Worksurface_Width") - this.slantOffsetX;
    }
    
    @Override
    public float getRightDepth() {
        return this.getAttributeValueAsFloat("ICD_Scalable_Worksurface_Depth") - this.slantOffsetY;
    }
}
