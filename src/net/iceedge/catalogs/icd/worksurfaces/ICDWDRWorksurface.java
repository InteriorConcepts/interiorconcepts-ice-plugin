...
public class ICDWDRWorksurface extends ICDBasicWorksurface
{
    public ICDWDRWorksurface(final String s, final TypeObject typeObject, final OptionObject optionObject) {
        super(s, typeObject, optionObject);
    }
    
    public TransformableEntity buildClone(final ICDWDRWorksurface icdwdrWorksurface) {
        super.buildClone(icdwdrWorksurface);
        return (TransformableEntity)icdwdrWorksurface;
    }
    
    public TransformableEntity buildClone2(final ICDWDRWorksurface icdwdrWorksurface) {
        super.buildClone2(icdwdrWorksurface);
        return (TransformableEntity)icdwdrWorksurface;
    }
    
    public EntityObject frameClone(final EntityObject entityObject) {
        return this.buildFrameClone(new ICDWDRWorksurface(this.getId(), this.currentType, this.currentOption), entityObject);
    }
    
    @Override
    public Object clone() {
        return this.buildClone(new ICDWDRWorksurface(this.getId(), this.currentType, this.currentOption));
    }
    
    public EntityObject buildFrameClone(final ICDWDRWorksurface icdwdrWorksurface, final EntityObject entityObject) {
        super.buildFrameClone((TransformableTriggerUser)icdwdrWorksurface, entityObject);
        return (EntityObject)icdwdrWorksurface;
    }
