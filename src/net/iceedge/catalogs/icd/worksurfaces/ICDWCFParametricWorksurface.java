...
public class ICDWCFParametricWorksurface extends ICDCornerParametricWorksurface implements ICD2Widths2DepthsGrippable
{
    private static final float cutoutEdgeLength = 24.75f;
    ...
    public ICDWCFParametricWorksurface(final String s, final TypeObject typeObject, final OptionObject optionObject) {
    ...
    @Override
    public Object clone() {
        return this.buildClone(new ICDWCFParametricWorksurface(this.getId(), this.currentType, this.currentOption));
    }
    
    public ICDWCFParametricWorksurface buildClone(final ICDWCFParametricWorksurface icdwcfParametricWorksurface) {
        super.buildClone(icdwcfParametricWorksurface);
        icdwcfParametricWorksurface.calculateParameters();
        return icdwcfParametricWorksurface;
    }
    ...
    protected void calculateParameters() {
        ...
        final float chamferOffset = this.getChamferOffset(24.75f);
        ...
        this.topLeftFillet = new FilletParameter(this.leftEdge, this.topEdge, n4, true);
        ...
        final float n5 = n4 * (float)Math.sqrt(2.0) - n4 + 2.5f;
        final Vector3f vector3f2 = new Vector3f(1.0f, -1.0f, 0.0f);
        vector3f2.normalize();
        final Point3f point3 = ICDParametricWorksurface.pointAt(point3f, vector3f2, n5);
        this.cutoutSnapPointIndexMap.clear();
        this.cutoutSnapPointIndexMap.put(point3, 0);
        this.cutoutSnapPointIndexMap.put(key, 1);
        this.cutoutSnapPointIndexMap.put(key2, 2);
        this.updateSnappedCutouts(point3, 0);
        this.updateSnappedCutouts(key, 1);
        this.updateSnappedCutouts(key2, 2);
        this.cutoutSnapPoints.clear();
        this.addCutoutSnapPoint(point3);
    ...
    public float getWidth1Min() {
        ...
        return Math.max(minimumValueFromAttribute, MathUtilities.roundToNearestDecmial(this.getAttributeValueAsFloat("ICD_Parametric_Depth2") + this.getChamferOffset(24.75f) + 2.0f + 0.25f, 0.5f));
    }
    ...
    public float getWidth2Min() {
        ...
        return Math.max(minimumValueFromAttribute, MathUtilities.roundToNearestDecmial(this.getAttributeValueAsFloat("ICD_Parametric_Depth1") + this.getChamferOffset(24.75f) + 2.0f + 0.25f, 0.5f));
    }
